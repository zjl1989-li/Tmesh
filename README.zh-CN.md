...
| **本地 CLI agent** | 每轮拉起本地 CLI（探测含一次真实对话回路） | `codex exec` 走本地中转 |
...
- **L0 工作记忆**：上下文按 `ctxBudgetChars` 预算裁剪，超长的老对话自动瘦身，token 不再失控；群里发 `/clear` 立即重置模型上下文（聊天记录照常可见，只是不再喂给成员）
...
- **用量台账**：按 agent 记每日/每月 tokens + 轮次——API 类分 prompt/completion，CLI 类记总量，agent 卡片即看即警
